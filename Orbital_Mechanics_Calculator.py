...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("rocket.png"), QtGui.QIcon.Selected, QtGui.QIcon.On)
    ...
    # Disable Injection at: options for orbital velocity calculation
    ...
    # Calculate delta V or orbital velocity from provided data
    def calculate(self):
    ...
    def check_numerical(self):
        ...
        if self.radius_peryapse.isnumeric() and self.radius_apoapse.isnumeric() and self.radius_calculation.isnumeric():
            ...
            if self.radius_peryapse[0] == '-' or self.radius_apoapse[0] == '-' or self.radius_calculation[0] == '-':
                is_numeric = True
            is_numeric = False
    ...
    # Set object mass and radius according to chosen primary body
    def get_data(self):
    ...
    def get_equation_orbital(self):
        ...
        # Check for logical errors in inputs
        if Rc < -1*self.radius or Ra < -1*self.radius or Rp < 0:
    ...
    def get_equation_delta(self):
        ...
        unit_circular = 'm/s'
        # Check for logical errors in inputs
        if Rf < -1*self.radius or Ra < -1*self.radius or Rp < 0:
            self.label_result.setText(f"Entered radius lower than radius of primary body.")
            self.label_circ_orbit_inj_result.setText(f"You just tried to warp drive.")
            self.line_orbit_height_input.clear()
            self.line_apoapse_input.clear()
            self.line_peryapse_input.clear()
            return
        # Calculate delta V for peryapse
